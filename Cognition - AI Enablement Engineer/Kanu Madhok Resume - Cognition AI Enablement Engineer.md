...
_Text source for the .docx / .pdf. Tailored for Cognition — AI Enablement Engineer. In the formatted version, city · date sit right-aligned to the page edge._
...
Agentic AI: sub-agent orchestration, MCP (Model Context Protocol) server design, tool- & function-calling agents, context & prompt engineering, multi-model routing across providers, headless cron agents, event-driven agent workflows

Evaluation & reliability: golden-query evaluation suites, deterministic SQL guardrail rules, context-isolated validation, self-healing execution with retry budgets, decision logging & monitoring

RAG & retrieval: FAISS + BM25 + RRF, ChromaDB semantic memory
