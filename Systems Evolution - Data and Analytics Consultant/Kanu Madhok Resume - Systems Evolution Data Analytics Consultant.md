...
> **NOTE:** JD body is empty in this folder (LinkedIn paste pending). This resume is built from the role title alone (Data and Analytics Consultant at a consulting firm). Re-tailor after pasting the full JD.
...
- Designed and shipped a self-service analytics agent over 67 BigQuery tables with sub-agent orchestration and context-isolated validation; 8 deterministic SQL rules, 23 golden-query test cases. Approved by Sr. Director and product leadership and currently the only AI skill in active use by business teams on Data Ventures.
...
- Built a headless KPI monitor on a 6-hour cron that detects threshold breaches, runs root-cause analysis, and posts Slack alerts; state persists to BigQuery.
