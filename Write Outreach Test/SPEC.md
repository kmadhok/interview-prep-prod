...
**Owner:** Kanu Madhok
**Purpose.** Run the same task — *improve the `write-outreach` skill, then generate cold-recruiter emails with it* — across multiple provider models (Codex / Claude / Gemini), each in its own git worktree, and pick the model whose improved skill produces emails closest to a single **gold-standard email**.

This is a benchmark, not a feature. The deliverable of a *winning run* is an improved `write-outreach` skill. The deliverable of *this folder* is the harness that decides which run won.
