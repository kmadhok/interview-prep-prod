...
_Text source for the .docx / .pdf. Tailored for Capgemini — AI Product Engineer, Agentic AI Platforms (Financial Services). In the formatted version, city · date sit right-aligned to the page edge._
...
- Designed and shipped a production self-service analytics agent over 67 BigQuery tables — a multi-agent architecture with planner / executor / reviewer / critic sub-agents and context-isolated validation (Context Researcher → SQL Drafter → Validator → Devil's Advocate), governed by 8 deterministic guardrail rules and a 23-case golden-query evaluation suite. Approved by Sr. Director and product leadership and currently the only AI skill in active use by business teams on Data Ventures. Eliminated the analyst as the bottleneck for routine asks.
- Built a hybrid orchestrator on top of the agent above. Routes business questions through two backends in parallel: a CubeJS semantic-layer skill (co-built with the DS and product team) and the context-engineered agent itself. Compares outputs, reasons about disagreements, and feeds discrepancies back to refine the semantic layer's definitions over time.
...
_Capstone engagement with FTI Consulting, a global financial & business advisory firm_

- Built a RAG-to-proposal generator that automated drafting of client engagement proposals for FTI's advisory teams; deployed on AWS EC2, cut proposal turnaround ~30%; Best in Show — UChicago MSADS Capstone.
...
- Rapidly prototyped and live-deployed a text-to-SQL copilot (Streamlit + LangChain + Gemini 2.5 + RAG over historical queries, BigQuery execution, auto-visualization): live demo — https://sql-rag-frontend-simple-481433773942.us-central1.run.app/

---

## SKILLS

Agentic AI: multi-agent architecture (planner / executor / reviewer / supervisor patterns), sub-agent orchestration, MCP (Model Context Protocol) server design, tool- & function-calling agents, memory-enabled agents, context & prompt engineering, event-driven agent workflows, headless cron agents

Evaluation & reliability: golden-query evaluation suites, deterministic guardrail rules, offline testing of agent flows, output consistency checks, self-healing execution, decision logging & monitoring

RAG & retrieval: FAISS + BM25 + RRF, Pinecone, ChromaDB semantic memory, multi-model routing across providers

LLMs & frameworks: Claude (Opus / Sonnet / Haiku), Gemini (2.5 Pro / Flash), OpenAI, LangChain, Claude Code (daily — via Wibey, Walmart's internal Claude Code fork)

Engineering & delivery: Python, FastAPI, Cloud Run, BigQuery scheduled queries, Slack webhooks, cron, Playwright SSO, JSONL state stores
