...
4. **Cadence interaction:** saved-jobs can surface many roles at once; keep the one-role-per-run bound and the `>=4 unsent` staging pause so the draft-review queue doesn't flood.
