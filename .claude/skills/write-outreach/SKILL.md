...
8. **Save to Gmail draft** — runs only AFTER the human-writing pass AND the Format check gate pass for the message in question; never draft from an unchecked message. Use `mcp__claude_ai_Gmail__create_draft` — it saves to the Drafts folder and sends nothing. This skill DRAFTS ONLY; never call any send tool.
   ...
   - **Emit BOTH `body` and `htmlBody`.** Passing only plain-text `body` makes Gmail soft-wrap the long signature line into several ugly lines and render full `http(s)://…` URLs as literal text (which Gmail then copy-wraps into `google.com/url?q=…` redirects). Always pass:
     - `body` = the plain-text draft verbatim (the fallback, unchanged from the format-checked message).
     - `htmlBody` = the same content rendered as HTML: wrap each paragraph in `<p>…</p>`; render the signature as one `<p>` with `<br>` between its lines; turn every link into a short-display-text anchor — `<a href="https://<user_linkedin from profile.yaml>"><user_linkedin from profile.yaml></a>`, `<a href="https://<user_github from profile.yaml>"><user_github from profile.yaml></a>`, and the Live demo as `<a href="<full url>">…run.app</a>`. Use `https://` hrefs (never `http://`), keep the `·` separators as literal text between anchors, and change ONLY the rendering — never the wording, order, or signature block defined by `Outreach Templates.md`. HTML-escape any literal `&`, `<`, `>` in visible text.
   ...
   - **Fallback** — if `Verified Emails.md` doesn't exist, or the name isn't found, set To: = `<user_email from profile.yaml>` and record `{source:"outreach", kind:"unverified-recipient", detail:"no verified email in Verified Emails.md for <contact name> — To: set to <user_email from profile.yaml>; swap before send"}`. (Gmail's API needs at least one recipient — that's why the fallback uses the user's own address rather than an empty To:.)
