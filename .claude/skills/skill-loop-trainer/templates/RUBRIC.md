...
> Gold email's load-bearing edge, in the user's own words: **(1) relevance** — it ties to the actual JD
> *and* their current work, ending on a "this role is the closer match for what I do" pivot; and
...
| **relevance_match** | 0.20 | Hook + close tie to THIS role's actual JD and the user's current work, ending on a "closer match for what I do" style pivot. Specific to the req, not generic interest. | Generic ("interested in opportunities at your company"); no JD tie; could be sent to any company. |
| **voice** | 0.15 | First-person, conversational, reads like the user is talking. Zero corporate filler. | Stiff, templated, "to express my interest" register. |
...
| **sounds_like_user** | 0.04 | Ownership-forward, business-outcome framing, crisp numbers, zero hype words. Indistinguishable from the user's real writing. | AI-generic; hype words present. |

Weights sum to 1.00. **urgency_social_proof + relevance_match dominate (0.42 combined)** — they are
exactly the two traits the user named as why the gold email is gold, and the hardest for a model to fake
without fabricating.
...
- [x] Set the two top dimensions to the user's stated reasons: relevance-match + urgency/social-proof.
