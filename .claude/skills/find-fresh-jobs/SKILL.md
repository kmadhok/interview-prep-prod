...
description: Use this skill when Kanu Madhok wants a fresh batch of LinkedIn job listings to evaluate against his Job Search Target Profile. Surfaces N high-quality, recently-posted roles via the gated LinkedIn MCP, applies every filter in the Target Profile (geo-adjusted comp floor, seniority, hard-skip company list, archetype, leetcode-free preference, industry exclusions, post-read drop signals), dedupes against the current Pipeline so already-filed roles don't re-surface, and returns a compact evaluation-ready report. Defaults to N=10 jobs posted in the last 24 hours. Trigger language includes "find fresh jobs", "find me jobs", "pull jobs", "show me new jobs", "what's new on LinkedIn today", "run the job search", "morning job pulse", or any variant that asks for a batch of new listings to review. Args supported via natural language ("find me 5 jobs from the last 72h", "find me 15 fresh jobs"). Do NOT trigger when Kanu shares a single JD and wants it filed (use `interview-prep-intake`), wants the full apply-ready pipeline for one JD (use `jd-to-ready`), or wants to bulk-import jobs he already saved from LinkedIn (use `linkedin-saved-jobs-intake`). If ambiguous, assume he wants the default (N=10, 24h) and run it.
---

# Find Fresh Jobs — LinkedIn morning pulse

This skill is the repeatable "what's new for me on LinkedIn today" pass. It surfaces a fresh batch of roles that match Kanu's `Job Search Target Profile.md`, applies every filter in that file, dedupes against `Pipeline.md`, and returns a compact report he can scan in under 60 seconds.

Read `AGENTS.md` / `CLAUDE.md` at `/Users/kanumadhok/Documents/Claude/Projects/Interview Prep/` first. Workspace conventions there override anything here on conflict.
...
- A short "What I skipped and why" paragraph naming 2–3 dropped results so Kanu can sanity-check the filters
...
## Inputs (parse from Kanu's message)

- **N** — how many jobs to return. Default **10**. Override if Kanu says a number ("find me 5", "15 fresh ones", etc.)
- **Window** — how recent. Default **past 24 hours**. Override if Kanu says a window ("last 48h", "past 3 days", "this week")
- **Optional theme override** — if Kanu names a focus ("only data-agent stuff", "skip consulting", "FDE only"), apply it as a post-filter on top of the Target Profile
...
Always re-read the file — don't cache. Kanu tunes it.
...
- If the daemon isn't up, surface that to Kanu and stop — don't work around it
...
7. **Theme override (if Kanu provided one)** — drop anything that fails it
...
**Flag any widening in the final report.** Kanu wants to know if the day was thin.
...
End the report. Do not append next-step suggestions unless Kanu asked. He'll decide what to do with the list.

## Hard rules

- **Never parallelize MCP calls.** Sequential only, jittered, per `linkedin-mcp-operations`.
- **Never modify files in the workspace.** This skill is read-only — no edits to Pipeline.md, no new role folders, no memory writes. If Kanu wants to file a result, he'll invoke `interview-prep-intake` or `jd-to-ready` separately.
...
Kanu can scan the report in under 60 seconds and form an opinion on each role without clicking through. The "why it fits" bullets cite specific JD language, not generic praise. The "what I skipped" paragraph proves the filters are working (not silently passing every result). The skill takes 5–10 minutes wall-clock — most of it spent on jittered sequential MCP calls — and Kanu can interrupt at any point.

## When NOT to use this skill

- Kanu shares a single JD (PDF, URL, or pasted text) and wants it filed → use `interview-prep-intake`
- Kanu wants the full apply-ready package for one specific JD → use `jd-to-ready`
- Kanu pastes 3+ jobs from his LinkedIn Saved Jobs page → use `linkedin-saved-jobs-intake`
- Kanu wants to find contacts at a specific company (not jobs) → use `find-contacts`
- Kanu wants to research the recruiter for a role already filed → use `job-outreach`

If ambiguous between this skill and `linkedin-saved-jobs-intake`: the tell is *who did the curation*. If Kanu already saved jobs and is pasting them, it's `linkedin-saved-jobs-intake`. If he wants you to do the search-and-filter pass, it's this skill.
