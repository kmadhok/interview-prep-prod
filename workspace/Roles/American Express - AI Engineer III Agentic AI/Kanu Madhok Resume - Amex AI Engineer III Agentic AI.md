...
_End-to-end AI engineer who ships production agentic systems — tool-calling, RAG, and evaluation — with the deterministic validation, observability, and reproducibility that regulated environments demand._
...
- Built a registry-governed semantic layer over BigQuery that boxes the LLM into semantic parsing — one call, strict JSON intent, never SQL — with deterministic validation, value canonicalization, schema-validated structured outputs, and a deterministic SQL compiler, making answers reproducible and the only probabilistic step regression-testable. New or changed definitions pass a human review gate before entering the registry.
