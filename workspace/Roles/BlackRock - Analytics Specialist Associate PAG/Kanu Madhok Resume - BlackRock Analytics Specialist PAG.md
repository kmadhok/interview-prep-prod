...
- Evolved the analytics skill into a registry-governed semantic layer over BigQuery: a single LLM call maps the question to strict JSON intent (the model never writes SQL); a deterministic gate validates and canonicalizes intent before a deterministic compiler emits the SQL — same question, same answer, every time. New or changed definitions pass a human review gate before entering the registry.
...
Agent & evaluation stack: sub-agent orchestration, context engineering, RAG (FAISS + BM25 + RRF), evaluation suites (golden sets, human-in-the-loop validation), headless cron agents, event-driven architectures, decision logging / observability, MCP (Model Context Protocol) server design
