...
1. **Delivery: Drive, via a small Drive-API script on the PC — not the MCP.** Drive for Desktop is not installed on the PC. The claude.ai Drive MCP was smoke-tested 2026-07-01: it works interactively (created the folders — `Apply Queue/` = Drive ID `1UD10yiFNeuE2fFYD_cMXlQOEuHRNtPNk`, `Apply Queue/Applied/` = `1u4PkaEs6eo6sRwKkikRVqAtYnxisa84G`) but has **two disqualifying gaps for the automation**: its token expires and needs interactive re-auth (observed same day), and its toolset has **no move, delete, or update** — so the reconcile table (move to `Applied/`, remove dead, replace stale PDF in place) cannot be implemented on it. Build therefore uses `scripts/apply_packet/` with the Google Drive API and a **service account that the `Apply Queue/` folder is shared with** — full CRUD, no expiring interactive token, headless-proof. The MCP stays useful for interactive sessions (inspection, one-off fixes). Fail-loud requirement stands: any upload/reconcile failure surfaces as a digest line, never a silent skip (invariant 4).
