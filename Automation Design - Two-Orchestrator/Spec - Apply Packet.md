...
1. **Delivery: Drive via `rclone` on the PC — set up and CRUD-verified 2026-07-01.** Drive for Desktop is not installed on the PC. The claude.ai Drive MCP was smoke-tested: it works interactively (created the folders — `Apply Queue/` = Drive ID `1UD10yiFNeuE2fFYD_cMXlQOEuHRNtPNk`, `Apply Queue/Applied/` = `1u4PkaEs6eo6sRwKkikRVqAtYnxisa84G`) but is **disqualified for the automation**: expiring interactive token (observed twice same day), no move/delete/update in its toolset (reconcile table unimplementable), and any MCP upload drags the PDF through the model as base64 (~35k+ tokens/role + transcription-corruption risk). Two alternatives were rejected: a **Drive-API service account** (setup burden Kanu vetoed) and a **Gmail nested-label packet** (feasible — attachment + nested-label support proven by probe — but ~8 taps at apply vs Drive's 3–4, no `delete_draft` so dead packets accumulate in Drafts, and the same base64-through-model cost). **The build**: `rclone` installed via winget, remote **`gdrive`** (scope=drive, token auto-refreshes, on PATH for new shells); upload/`moveto`/`delete` all verified against the real folder. The reconcile pass shells out to rclone — the PDF never enters model context. The Drive MCP stays useful for interactive sessions (inspection, one-off fixes). Fail-loud requirement stands: any upload/reconcile failure surfaces as a digest line, never a silent skip (invariant 4).
2. **`Application Profile.md` seeded and complete (2026-07-01)** — salary $130k+ base, U.S. citizen (no sponsorship required), 2-week notice, phone, LinkedIn + GitHub URLs, open to remote/in-person any location. No placeholders remain.
3. **ntfy topic: `job-auto-abcee`** (chosen 2026-07-01). ntfy topics are unauthenticated — the topic name *is* the password; only the iPhone app and the PC cron know it. Do not commit the topic name anywhere outside this design folder.
