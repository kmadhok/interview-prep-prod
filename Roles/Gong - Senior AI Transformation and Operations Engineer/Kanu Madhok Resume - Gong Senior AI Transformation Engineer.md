...
_End-to-end AI engineer who turns stakeholder ideas into working agentic workflows — shipping POCs that prove ROI and the governance, cost, and monitoring guardrails that keep enterprise AI consistent and scalable._
...
- Built a registry-governed semantic layer over BigQuery that makes enterprise AI consistent and governed — one LLM call to strict JSON intent (the model never writes SQL), deterministic validation, value canonicalization, and a deterministic SQL compiler, so answers are reproducible and the only probabilistic step is regression-testable. New or changed definitions pass a human review gate before entering the registry.
...
Agent stack: sub-agent orchestration, LangChain, MCP (Model Context Protocol) server design, context engineering, prompt engineering, prompt libraries, RAG (embeddings + FAISS + BM25 + RRF), vector databases (FAISS, ChromaDB), evaluation suites (golden sets, human-in-the-loop validation), multi-model routing across providers, headless cron agents, event-driven architectures, decision logging / observability
...
Data & Analytics: BigQuery, SQL, dbt, Python, PySpark, Airflow, Databricks, MySQL

Viz: Tableau, Power BI (DAX, XMLA TMSL), LookML, Streamlit, Matplotlib, React Recharts
