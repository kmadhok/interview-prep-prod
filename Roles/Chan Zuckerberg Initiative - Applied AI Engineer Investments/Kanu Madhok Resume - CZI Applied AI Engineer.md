...
- Built a registry-governed semantic layer over BigQuery with deterministic validation, value canonicalization, and a deterministic SQL compiler; data validation and a human review gate keep investment-grade reliability so the same question returns the same answer every time.
- Built a headless KPI monitor on a 6-hour cron that detects threshold breaches, runs root-cause analysis, and posts Slack alerts; state and decision logs persist to BigQuery for end-to-end observability and data-quality monitoring.
...
AI / applied: MCP server design, sub-agent orchestration, context engineering, prompt engineering, RAG (embeddings + FAISS + BM25 + RRF) and retrieval systems, LangChain, workflow automation, evaluation suites (golden sets, human-in-the-loop validation), multi-model routing across providers

Software / engineering: Python, FastAPI, Cloud Run, APIs, CI/CD (pytest, version control), event-driven architectures, decision logging / observability, JSONL state stores

Data: BigQuery, dbt, SQL, PySpark, Airflow, Databricks, MySQL; data pipelines, data validation / quality checks, structured + unstructured data prep
