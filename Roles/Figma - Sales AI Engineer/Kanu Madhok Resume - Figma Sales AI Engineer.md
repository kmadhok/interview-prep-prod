...
_End-to-end AI engineer who embeds with stakeholders, rapidly prototypes, and ships production-grade agentic workflows with measurable adoption and business impact._
...
Agent stack: MCP (Model Context Protocol) server design, sub-agent orchestration, context engineering, prompt engineering, agentic RAG (FAISS + BM25 + RRF), multi-model routing across providers, headless cron agents, event-driven architectures, evaluation suites (golden sets, human-in-the-loop validation), decision logging / observability

LLMs: Claude (Opus / Sonnet / Haiku via Wibey), OpenAI, Gemini (2.5 Pro / Flash via Vertex AI)
