...
- Built and shipped to production an autonomous Jira ticket-resolution agent: 6-gate triage, MCP-driven context gathering, agentic RAG with FAISS + BM25 retrieval over 11,000 historical SQL queries, and self-healing execution with a 10-retry budget. Closed 400+ data requests across 8 categories over 15 months — cut routine turnaround from 30–60 minutes to under 10 minutes each. Debugged and improved it across model output, retrieval, and system logic; stakeholders never knew an agent was answering them.
- Built a hybrid orchestrator on top of the analytics agent that routes business questions through two backends in parallel — a CubeJS semantic-layer skill (co-built with the DS and product team) and the context-engineered agent — then compares outputs, reasons about disagreements, and feeds discrepancies back to refine the semantic layer's definitions over time. Evolved it into a registry-governed semantic layer where a single LLM call maps the question to strict JSON intent (the model never writes SQL) before a deterministic gate and compiler produce reproducible answers, making the one probabilistic step regression-testable.
...
- Replaced ~20 distinct recruitment workflows I had been running by hand 1–2× weekly with end-to-end automation on a 68-table BigQuery platform; stratified-sampling pipelines recruited 280,000+ panelists across 29 categories with zero duplicates, turning a recurring 3-hour task into ~3 minutes of setup.
...
APIs / backend / glue: FastAPI, Cloud Run (containerized deploy), BigQuery scheduled queries, Slack webhooks, cron, Playwright SSO, JSONL state stores, CI/CD (pytest, version control)

Data: BigQuery, dbt, SQL, Python, PySpark, Airflow, Databricks, MySQL

Viz: Streamlit, Power BI, Tableau, Matplotlib, React Recharts
