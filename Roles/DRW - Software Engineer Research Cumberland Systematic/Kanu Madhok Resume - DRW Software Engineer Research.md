...
- Designed and shipped to production a self-service analytics system over 67 BigQuery tables with sub-agent orchestration and human-in-the-loop validation (Context Researcher → SQL Drafter → Validator → Devil's Advocate); 8 deterministic SQL rules and a 23-case golden-query evaluation suite testing correctness before release. Partnered directly with Product, Data Science, and business stakeholders to translate ambiguous questions into precise specs. Approved by Sr. Director and product leadership.
...
- Built a registry-governed semantic layer over BigQuery with strong type discipline: a single LLM call maps the question to strict JSON intent, a deterministic gate validates and canonicalizes it into a typed dataclass, and a deterministic compiler emits the SQL — same input, same output, every time, with the one probabilistic step isolated and regression-tested.
- Built and shipped an autonomous Jira ticket-resolution agent in modern, refactorable Python: 6-gate triage, MCP-driven context gathering, FAISS + BM25 retrieval over 11,000 historical SQL queries, self-healing execution with a 10-retry budget. Closed 400+ data requests across 8 categories over 15 months — cut routine turnaround from 30–60 minutes to under 10 minutes each.
- Built and deployed a headless KPI monitor on a 6-hour cron that detects threshold breaches, runs root-cause analysis, and posts Slack alerts; end-to-end observability via decision logging and BigQuery state persistence (no in-memory reliance). Designed a 3-tier event-driven v2 platform self-configuring via a BigQuery event registry — new event types ship without code deploys.
...
Data engineering: BigQuery, dbt, SQL, Python, PySpark, Airflow, Databricks, MySQL; large-table pipelines, stratified-sampling, BigQuery scheduled queries, JSONL state stores

Engineering rigor: modern refactorable Python, typed-dataclass validation, deterministic compilers, golden-set / human-in-the-loop evaluation suites, CI/CD (pytest, version control), decision logging / observability

Agent stack: MCP server design, sub-agent orchestration, context engineering, RAG (FAISS + BM25 + RRF), headless cron agents, event-driven architectures

LLMs: Claude (Opus / Sonnet / Haiku via Wibey), Gemini (2.5 Pro / Flash via Vertex AI), OpenAI

APIs / triggers / glue: FastAPI, Cloud Run, Slack webhooks, cron, Playwright SSO
