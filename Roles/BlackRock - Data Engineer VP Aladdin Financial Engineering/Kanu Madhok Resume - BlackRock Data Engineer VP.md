...
- Built a registry-governed semantic layer over BigQuery: YAML definitions plus SQLite vocabulary snapshots define legal dimensions, values, and table wiring; a deterministic gate validates and canonicalizes intent into a typed dataclass before a deterministic compiler emits the SQL. New or changed definitions pass a human review gate before entering the registry — governed data, reproducible by construction, and the only probabilistic step regression-testable.
...
- Built an autonomous Jira ticket-resolution agent that uses AI to drive efficiency: 6-gate triage, MCP-driven context gathering, FAISS+BM25 retrieval over 11,000 historical SQL queries, self-healing execution with a 10-retry budget. Closed 400+ data requests across 8 categories over 15 months — cut turnaround from 30–60 minutes per request to under 10 minutes each.
- Built a headless KPI monitor on a 6-hour cron that detects threshold breaches, runs root-cause analysis, and posts Slack alerts; state persists to BigQuery for end-to-end observability. Designed a v2 architecture as a 3-tier event-driven platform self-configuring via a BigQuery event registry, so new event types ship without code deploys.
...
Data & pipelines: BigQuery, dbt, SQL, Python, PySpark, Airflow, Databricks, MySQL; ETL / data curation, stratified-sampling pipelines, registry-governed semantic-layer design, deterministic SQL compilation

Engineering rigor: CI/CD (pytest, version control), event-driven architectures, headless cron jobs, decision logging / observability, JSONL state stores

Agent stack: MCP server design, sub-agent orchestration, context engineering, RAG (FAISS + BM25 + RRF), evaluation suites (golden sets, human-in-the-loop validation), multi-model routing across providers
