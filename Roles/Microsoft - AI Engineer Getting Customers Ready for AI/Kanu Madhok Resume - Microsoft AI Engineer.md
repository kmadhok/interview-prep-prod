...
- Evolved the analytics skill into a registry-governed semantic layer over BigQuery: YAML definitions plus SQLite vocabulary snapshots, a deterministic prompt builder, a single LLM call that maps the question to strict JSON intent (the model never writes SQL), a deterministic validation gate, and a deterministic SQL compiler — same question, same answer, every time, with the only probabilistic step regression-testable.
