...
_Text source for the .docx / .pdf. Tailored for Morningstar — Product AI Engineer (REQ-055251). In the formatted version, city · date sit right-aligned to the page edge._
...
AI dev tools: Claude Code (daily — via Wibey, Walmart's internal Claude Code fork), agentic coding workflows, rapid prototyping with limited infrastructure

Agent stack: MCP (Model Context Protocol) server design, sub-agent orchestration, context engineering, prompt engineering, RAG (FAISS + BM25 + RRF), LangChain, ChromaDB semantic memory, multi-model routing across providers, headless cron agents, event-driven architectures
