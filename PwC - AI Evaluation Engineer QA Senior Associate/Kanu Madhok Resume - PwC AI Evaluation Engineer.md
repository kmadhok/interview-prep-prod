...
_Text source for the .docx / .pdf. Tailored for PwC — US Tech, AI Evaluation Engineer (QA), Senior Associate (People Tech & AI). Skill ordering leads with Evaluation & QA because that's the JD's lead theme; agent-building skills support. In the formatted version, city · date sit right-aligned to the page edge._
...
- Designed and shipped a production self-service analytics agent over 67 BigQuery tables — multi-agent architecture with Context Researcher → SQL Drafter → Validator → Devil's Advocate sub-agents and context-isolated validation, **governed by 8 deterministic guardrail rules and a 23-case golden-query evaluation suite**. Approved by Sr. Director and product leadership and currently the only AI skill in active use by business teams on Data Ventures.
...
- Rapidly prototyped and live-deployed a text-to-SQL copilot (Streamlit + Gemini 2.5 + RAG over historical queries, BigQuery execution, auto-visualization): live demo — https://sql-rag-frontend-simple-481433773942.us-central1.run.app/

---

## SKILLS

Evaluation & reliability: golden-query evaluation suites, deterministic guardrail rules, offline testing of agent flows, output consistency checks, self-healing execution, decision logging & monitoring, A/B testing across LLM providers
...
RAG & retrieval: FAISS + BM25 + RRF, ChromaDB semantic memory, multi-model routing across providers

LLMs & frameworks: Claude (Opus / Sonnet / Haiku — daily via Wibey, Walmart's internal Claude Code fork), Gemini (2.5 Pro / Flash via Vertex AI), OpenAI
